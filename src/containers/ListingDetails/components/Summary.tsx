import { Alert, Box, Divider, Flex, Text, VStack } from "@chakra-ui/react";
  ...
  return (
    <VStack align={"start"}>
      {currentUser?.userType === roles.host && (
        <>
          {listing?.interestedParties.length > 0 ? (
            <Alert variant="subtle" status="info" rounded="lg">
              <Text color="blue" cursor="pointer" textAlign={"left"} _hover={{ textDecoration: "underline" }}>
                {listing?.interestedParties.length} {listing?.interestedParties.length === 1 ? "person" : "people"}{" "}
                {listing?.interestedParties.length === 1 ? "is" : "are"} interested in your property
              </Text>
            </Alert>
          ) : null}
        </>
